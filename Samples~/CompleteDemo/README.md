...
Open `Greeting.dlg` to inspect the same fields used in normal authoring: Dialogue and Choice text is written directly as Inline or Localized content, while connected local/shared/custom values override and preserve those defaults. Turn on **Runtime Debug** only while the sample is playing to inspect its current node, last transition, and resolved value previews.
